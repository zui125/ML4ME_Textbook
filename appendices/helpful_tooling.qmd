...
Running python train.py gives a cryptic type error at the loss computation. The problem is that the model outputs `float32` predictions while y is `float64`. This causes a mismatch in the loss computation. The fix is to change the lines to:
...
For this, we recommend using experiment trackers, such as [weights and biases](#hyperparameters). See how we do it in EngiOpt: for [hyperparameters definition](https://github.com/IDEALLab/EngiOpt/blob/main/engiopt/cgan_1d/cgan_1d.py#L50-L88) and [logging](https://github.com/IDEALLab/EngiOpt/blob/main/engiopt/cgan_1d/cgan_1d.py#L221), and logging [losses](https://github.com/IDEALLab/EngiOpt/blob/main/engiopt/cgan_1d/cgan_1d.py#L330-L337) for example.
